...
std::vector<double> multiply_rows(const std::vector<double>& left_rows,
    ...
    for (int row = 0; row < row_count; ++row) {
        for (int step = 0; step < order; ++step) {
            const double cell = left_rows[static_cast<std::size_t>(row) * order + step];
            for (int column = 0; column < order; ++column) {
                result[static_cast<std::size_t>(row) * order + column] += cell * right[static_cast<std::size_t>(step) * order + column];
            }
        }
    }
    return result;
